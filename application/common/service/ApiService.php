...
use app\common\model\common\Options;
...
//        import('Page');
        ...
        //根据参数生成查询条件
        $where[] = ['term_relationships.status','=',1];
...
//        $where['posts.post_status'] = 1;
        ...
        if (isset($tag['cid'])) {
            ...
            $where[] = ['term_relationships.term_id', 'in', $tag['cid']];
        }

        if (isset($tag['ids'])) {
            $tag['ids']=explode(',', $tag['ids']);
            $tag['ids']=array_map('intval', $tag['ids']);
//            $where['term_relationships.object_id'] = $tag['ids'];
            $where[] = ['term_relationships.object_id', 'in', $tag['ids']];
        }
...
//        var_dump($where);
        ...
        $join = '__POSTS__ posts';
        $con = 'term_relationships.object_id = posts.id';
        $join2= '__USERS__ users';
        $con2= 'posts.post_author = users.id';

        $term_relationships_model= db("TermRelationships");
        $content=array();
...
//                ->select();
    ...
    public static function postsByTermId($term_id,$tag,$where=array()){
        ...
        $term_ids=db("Terms")->where("status=1 and ( term_id=$term_id OR path like '%-$term_id-%' )")->order('term_id asc')->getField('term_id',true);
...
//            ->where("status=1 and ( term_id=$term_id OR path like '%-$term_id-%' )")
            ->order('term_id asc')
            ->column('term_id');

        $where[]= ['term_relationships.term_id', 'in' ,$term_ids];
...
//        $where['posts.post_title'] = array('like',"%$keyword%");
    ...
    public static function post($post_id,$tag){
        ...
        $post=db('Posts')->field($field)->where($where)->find();
    ...
    public static function term($term_id){
        ...
        if(empty($terms)){
            ...
            cache('all_terms',$mterms);
            return isset($mterms["t$term_id"]) ? $mterms["t$term_id"] : null;
//            return $mterms["t$term_id"];
        }else{
            return isset($terms["t$term_id"]) ? $terms["t$term_id"] : null;
        }
    }
...
//        }
    ...
    public static function siteInfo(){
        $opt = Options::where("option_name='site_options'")->find();
        if($opt){
            $opt = json_decode($opt['option_value']);
        }
        if($opt){
            return $opt;
        }
