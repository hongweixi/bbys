...
class MenuModel extends CommonModel {
    ...
    public function menu_json()
    {
        ...
        if (count($pp) >= 2) {
            foreach ($pp as $k => $p) {
                ...
                if (stripos($url, $p['url']) === false) {
                    unset($pp[$k]);
                }
            }
        }
        foreach ($data as &$vv) {
            foreach ($vv['items'] as $kk1 => &$vv1) {
                if(!isset($pp[$kk1])){
                    $vv1['active'] = '';
                }
                foreach ($vv1['items'] as $kk2 => &$vv2) {
                    if(!isset($pp[$kk2])){
                        $vv2['active'] = '';
                    }
                }
    ...
    //取得树形结构的菜单
    public function get_tree($myid, &$pp = [], $parent = "", $Level = 1) {
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $a) {
                ...
                $url = $_SERVER['REQUEST_URI'];
                $array['active'] = '';
                if(stripos($url, $array['url']) !== false){
                    $array['active'] = 'active';
                    $pp[$id . $name] = $array ;
                }else{
                    // 解析URL
                ...
                $ret[$id . $name] = $array;
                $child = $this->get_tree($a['id'], $pp, $id, $Level);
